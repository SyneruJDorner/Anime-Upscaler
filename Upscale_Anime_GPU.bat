...
for /r %%i in (*) do (
	if not exist "%root_folder%Frames\Created\%%~nxi" (
		%execpath% -y upconv_7_anime_style_art_rgb --tta 0 -p gpu -n 3 -s 2.00 -m noise_scale -q 0 -no_overwrite 1 -i "%root_folder%Frames\Extracted\%%~nxi" -e bmp -o "%root_folder%Frames\Created\%%~nxi" >nul
		cls
	) else (
